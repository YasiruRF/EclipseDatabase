        ...
        if validate_bib_id(search_bib):
            ...
            if student:
                ...
                with st.container():
                    ...
                    - **Registered:** {student.get('created_at', 'N/A')[:10]}
                    """)
            else:
...
def show_all_students(db: DatabaseManager):
    ...
    with col1:
        search_term = st.text_input("Search students", placeholder="Search by name, Curtin ID, or Bib ID", key="all_students_search")
    
    with col2:
        house_filter = st.selectbox("Filter by House", options=["All"] + HOUSES, key="all_students_house_filter")
    
    with col3:
        gender_filter = st.selectbox("Filter by Gender", options=["All"] + GENDER_OPTIONS, key="all_students_gender_filter")
...
def filter_students(students, search_term="", house_filter="All", gender_filter="All"):
    ...
    if house_filter != "All":
        filtered = [student for student in filtered if student["house"] == house_filter]
    if gender_filter != "All":
        filtered = [student for student in filtered if student.get("gender", "Not specified") == gender_filter]
    return filtered
